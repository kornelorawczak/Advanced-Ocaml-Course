...
type comp =
  | Value of value
  | Exn of ident (* Dodano identyfikator wyjątku *)
...
let rec eval_env (env : env) (e : expr) : comp =
  ...
  | Snd e ->
      ...
      | _ -> raise Type_error)
  | Raise label -> Exn label (* Dodano zgłaszanie wyjątków z etykietami *)
  | Try(e1, label, e2) ->
      (match eval_env env e1 with
      | Exn l -> (* Dodano obsługe case wyjątka z etykietą po 'with' *)
        if l = label then eval_env env e2 else Exn l
      | Value v -> return v)

let eval_prog p =
  match eval_env M.empty p with
  | Exn "MyExn" -> raise MyExn
  | Exn label -> failwith ("Unhandeled exception " ^ label)
  | Value v -> v
