...
let rec eval_env (env : env) (e : expr) : comp =
  ...
  | Snd e ->
      ...
      (match v with
       ...
       | _ -> Exn "Type error")
  | Raise -> Exn "User raised exception"
  | Try(e1, e2) ->
      (match eval_env env e1 with
       | Exn _  -> eval_env env e2
       | Value v -> return v)

let eval_prog p =
  match eval_env M.empty p with
  | Exn msg -> raise (CustomException msg)
  | Value v -> v
...
let example_expr1 = Binop (Add, Fun ("x", Var "x"), Int 42);;

try
  let result1 = eval_prog example_expr1 in
  print_value result1
with
| CustomException msg -> print_endline ("Exception caught: " ^ msg);;