...
(* Funkcja która zwraca wszystkie sensowne pod wyrażenia, czyli takie które przejdą check_expr *)
...
let rec find_let (e : expr) : expr option = 
  ...
  | Binop(op, e1, e2) -> 
    let find1 = find_let e1 in 
    if find1 != None then find1 
    else find_let e2
  | If(p, t, e) -> 
    let find1 = find_let p in 
      if find1 != None then find1 
      else let find2 = find_let t in 
        if find2 != None then find2 
        else find_let e  

(* Funkcja replace zamienia wyrażenia a-równoważne old_expr na new_expr w wyrażeniu expr,
   o ile te podwyrażenia znalezion nie mają zmiennej związanej z listy bound *)
