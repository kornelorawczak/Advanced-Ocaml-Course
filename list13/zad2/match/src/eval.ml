...
and value =
  ...
  | VCtor of cname * value list (* ZAD2 *)
...
and match_patterns env (vs : value list) (ps : pattern list) = (* ZAD2 *)
  match vs, ps with 
  | [], [] -> Some env 
  | v :: vs', p :: ps' -> 
    (match match_pattern env v p with 
    | None -> None 
    | Some env' -> match_patterns env' vs' ps')
  | _ -> None
...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | Int n -> VInt n
  | Bool b -> VBool b
  | Ctor(c, es) -> VCtor(c, List.map (eval_env env) es) (* ZAD2 *)
  | If (p, t, e) ->
...
let rec string_of_value v =
  ...
  | VCtor(c, vs) -> c ^ "(" ^ string_of_values vs ^ ")" (* ZAD2 *)
