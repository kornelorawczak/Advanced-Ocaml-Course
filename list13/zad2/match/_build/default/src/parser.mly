...
base:
  | i = INT { Int i }
  | x = IDENT { Var x }
  | LPAREN; RPAREN { Ctor("Unit", []) } (* ZAD2 *)
  | LPAREN; e1 = mixfix; COMMA; e2 = mixfix; RPAREN { Ctor("Pair", [e1; e2]) } (* ZAD2 *)
  | LPAREN; e = mixfix; RPAREN { e }
  | TRUE { Bool true }
  | FALSE { Bool false }
  | c = CNAME; LPAREN; es = expr_list; RPAREN { Ctor(c, es) } (* ZAD2 *)
...
pattern:
  ...
  | TRUE    { PBool true }
  | FALSE   { PBool false }
  | c = CNAME; LPAREN; ps = pattern_list; RPAREN { PCtor(c, ps) } (* ZAD2 *)
  | c = CNAME; LPAREN; RPAREN { PCtor(c, []) }
  ;

pattern_list:
  | p = pattern; COMMA; ps = pattern_list { p :: ps }
  | p = pattern { [p] }
  ;
