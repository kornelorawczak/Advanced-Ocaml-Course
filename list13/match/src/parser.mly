...
%token TRY WITH RAISE MATCH END UNDERSCORE WHEN AMP
%token FST SND
%token EOF
...
clause:
  ...
  | p=pattern; WHEN; w=mixfix; ARR; e=mixfix { (PWhen(p, w), e) } (* ZAD3 *)
  ;

pattern:
  | p1=pattern; COMMA; p2=pattern { PPair(p1, p2) }
  | p1=pattern; BAR; p2=pattern { POr(p1, p2) } (* ZAD4 *)
  | p1=pattern; AMP; p2=pattern { PAnd(p1, p2) } (* ZAD5 *)
  | p=pattern_app { p }
  ;
