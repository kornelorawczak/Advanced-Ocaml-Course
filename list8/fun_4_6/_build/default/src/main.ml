...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | LetRec (f, x, e1, e2) -> (
    let closure = VRClosure(f, x, e1, env) in
    let new_env = M.add f closure env in
    eval_env new_env e2
  )    
  ...
  | App (e1, e2) ->
      ...
      | VRClosure (f, x, e1, env') as vr, v -> eval_env (M.add f vr (M.add x v env')) e1 
...
(*
interp "let rec f x = if x < 3 then 1 else f (x - 2) + f (x - 1) in f 10";;

interp "let rec f x = if x = 1 then 1 else x * f (x - 1) in f 5";;
*)