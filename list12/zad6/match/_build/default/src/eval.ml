...
let rec match_pattern env v p (bound : cname list) =
  match v, p with
  ...
  | VCtor(c1, v), PCtor(c2, p) when c1 = c2 ->
    if cname_in_list c1 bound then raise Several_used else 
    match_pattern env v p (c1 :: bound)
  | _, PCtor _ -> None
  | VPair(v1, v2), PPair(p1, p2) -> if p1 = p2 then raise Several_used else 
    (match match_pattern env v1 p1 bound with
    | None -> None
    | Some env -> match_pattern env v2 p2 bound)
  | _, PPair _ -> None
  | _, PAs(p, x) -> 
    (match match_pattern env v p bound with
